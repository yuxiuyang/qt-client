...
AnalyseEcg::~AnalyseEcg() {
	// TODO Auto-generated destructor stub
}
void AnalyseEcg::handle(const BYTE* buf,int len){
    addBuf(buf,len);
    open_block();
}
int AnalyseEcg::open_block(){
    int ix=0;
...
void AnalyseEcg::addBuf(const BYTE* buf,int len){
...
bool AnalyseEcg::anal_pag(const BYTE* buf,const int len){
    switch(buf[2]){
    case Data_Msg:
...
bool AnalyseEcg::checkData(const BYTE* buf,const int len,const BYTE value){
    BYTE sum=0x00;
    for(int i=0;i<len;i++){
        sum += buf[i];
    }
...
void AnalyseEcg::anal_DataPag(const BYTE* buf,int len){
